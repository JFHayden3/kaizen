...
IKSetup IKSetup::build_humanoid() {
    ...
    // Spine and pelvis cones: flexible BJJ athlete range (±45° per segment)
    setup.constraints[HumanBone::SPINE_MID] =
        ConeLimit{{0.0f, 1.0f, 0.0f}, glm::radians(45.0f)};
    ...
    // Ankle cones: 45° half-angle for BJJ foot mobility (hooks, foot-on-hip, guard)
    setup.constraints[HumanBone::FOOT_L] =
        ConeLimit{{0.0f, 0.0f, -1.0f}, glm::radians(45.0f)};
    setup.constraints[HumanBone::FOOT_R] =
        ConeLimit{{0.0f, 0.0f, -1.0f}, glm::radians(45.0f)};

    // Wrist cones: 60° half-angle for grip and framing flexibility
    setup.constraints[HumanBone::HAND_L] =
        ConeLimit{{1.0f, 0.0f, 0.0f}, glm::radians(60.0f)};
    setup.constraints[HumanBone::HAND_R] =
        ConeLimit{{-1.0f, 0.0f, 0.0f}, glm::radians(60.0f)};

    // Toe hinges: curl down for hooks (-60°), slight extension up (+30°)
    setup.constraints[HumanBone::TOES_L] =
        HingeLimit{{1.0f, 0.0f, 0.0f}, glm::radians(-60.0f), glm::radians(30.0f)};
    setup.constraints[HumanBone::TOES_R] =
        HingeLimit{{1.0f, 0.0f, 0.0f}, glm::radians(-60.0f), glm::radians(30.0f)};

    // Finger hinges: curl for grips (-90°), slight extension (+10°)
    setup.constraints[HumanBone::FINGERS_L] =
        HingeLimit{{0.0f, 0.0f, 1.0f}, glm::radians(-90.0f), glm::radians(10.0f)};
    setup.constraints[HumanBone::FINGERS_R] =
        HingeLimit{{0.0f, 0.0f, -1.0f}, glm::radians(-90.0f), glm::radians(10.0f)};
